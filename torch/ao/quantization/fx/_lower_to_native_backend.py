...
def _lower_weighted_ref_module(model: QuantizedGraphModule, ref_class: Type[torch.nn.Module]) -> QuantizedGraphModule:
    ...
    """
    if ref_class not in LOWER_MODULE_MAP and ref_class not in FUSED_MODULES:
    ...
    for n in model.graph.nodes:
        ...
        # For fused modules, TODO: write this comment
        if ref_class in FUSED_MODULES:
...
def _lower_to_native_backend(model: QuantizedGraphModule) -> QuantizedGraphModule:
    ...
    """
    for ref_class in list(LOWER_MODULE_MAP.keys()) + list(FUSED_MODULES):
